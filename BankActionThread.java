...
public class BankActionThread extends ActionThread
{
    ...
    public void executeApplication()
    {
        while(!theEvents.isEmpty()){
            ...
            // remove next event from queue and process it
            if(nextEvent != null){
                // remove event from queue and process
                nextEvent = theEvents.remove();
                nextEvent.process();

                // update time and next event action
                myReport.updateTime(nextEvent.getTime());
                lastEventReport = nextEvent.getDescription();
                SimulationEvent next = theEvents.peek();
                if(next != null){
                    nextEventAction = next.getDescription();
                }
