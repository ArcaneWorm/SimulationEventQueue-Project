...
public class Teller
{
    ...
    public Teller(String name,int maxTimeToHelpCustomer, SimulationEventQueue mySimulationEventQueue,
                            ...
                            Report toReportTo)
    {
        ...
        theEventQueue = mySimulationEventQueue;
        sharedRandomGenerator = simulationRandomGenerator;
        theLine = servicesLine;
        reportServices = toReportTo;
        
        // initial check
        theEventQueue.add(new CheckForCustomerEvent(0.0, "Teller "+myName+" starts checking the line"));
    ...
    public class CheckForCustomerEvent extends SimulationEvent
    {
    	...
    	{
            if(!theLine.isEmpty()) {
                // remove customer from line to serve them
                Customer customer = theLine.dequeue();
                serve(customer);
                // update report
                postActionReport = ("Teller "+myName+" served "+customer.getName());
                serving = null;
            }
            else {
                postActionReport = ("Teller "+myName+" found the line empty.");
                serving = null;
            }

            // schedule next check for customer
